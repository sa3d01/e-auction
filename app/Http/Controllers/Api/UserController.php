...
class UserController extends MasterController
{
    ...
    {
        $validator = Validator::make($request->only('phone_details','phone'),['phone' => 'required|max:10|regex:/[0-9]{10}/'],$this->validation_messages());
        if ($validator->fails()) {
            return $this->sendError($validator->errors()->first());
        }
        $user = User::where(['phone_details->country_key'=>$request['phone_details']['country_key'],'phone' => $request['phone']])->first();
        if (!$user) {
    ...
    {
        $validator = Validator::make($request->only('phone_details','phone','activation_code'),['activation_code'=>'required','phone' => 'required|max:10|regex:/[0-9]{10}/'],$this->validation_messages());
        if ($validator->fails()) {
            return $this->sendError($validator->errors()->first());
        }
        $user = User::where(['phone_details->country_key'=>$request['phone_details']['country_key'],'phone' => $request['phone']])->first();
        ...
        if ($user->activation_code===$request['activation_code']){
            $data= new UserResource($user);
            $user->update(['activation_code'=>null,'phone_verified_at' => Carbon::now()]);
            $token = auth()->login($user);
            return $this->sendResponse($data)->withHeaders(['apiToken'=>$token,'tokenType'=>'bearer']);
        }else{
